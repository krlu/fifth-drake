...
class MongoDBHandler(){
  ...
  *********************************************************************************************************************/
  ...
  /**
    ...
    * @return List[GameState]
    */
  def getGameWindow(gameKey : RiotId[Game], begin : Duration, end : Duration): List[GameState] = {
    val filter = and(gte("t", begin.toMillis), lte("t", end.toMillis))
    val ob: FindObservable[Document] = database.getCollection("game_" + gameKey.id.toInt).find(filter)
    val docs = results[Document](ob)
    docs.map(doc => getGameState(doc)).toList
  }

  def getGameState(doc :Document) : GameState = {
