...
view model =
    let
        ...
        sideToDisplays side =
            ( sideToTeamDisplay side, sideToPlayerDisplay side )
        ...
        minimap =
            Minimap.view model.minimap model.game.data model.timestamp

        bluePlayers =
            model.game.data.blueTeam.players

        redPlayers =
            model.game.data.redTeam.players

        allPlayers =
            getPlayerIdsAndIgns bluePlayers redPlayers

        tagCarousel =
            TagCarousel.view model.tagCarousel allPlayers |> Html.map TagCarouselMsg
...
getPlayerIdsAndIgns bluePlayers redPlayers =
    let
        blueData =
            bluePlayers |> Array.map (\player -> ( player.id, player.ign ))

        redData =
            redPlayers |> Array.map (\player -> ( player.id, player.ign ))
    in
