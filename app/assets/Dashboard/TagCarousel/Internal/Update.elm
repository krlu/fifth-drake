    ...
    case msg of
        ...
        SaveTag ->
            let
                ...
                category =
                    model.tagForm.category

                description =
                    model.tagForm.description
            in
        ...
        CreateDescription description ->
            let
                ...
                newTagForm =
                    { oldTagForm | description = description }
            in
                ( Nothing, { model | tagForm = newTagForm }, Cmd.none )
        ...
        AddPlayers id ->
            let
                ...
                oldIdsList =
                    oldTagForm.selectedIds

                newIdsList =
                    if List.member id oldIdsList then
                        let
                ...
                newTagForm =
                    { oldTagForm | selectedIds = newIdsList }
            in
                ( Nothing, { model | tagForm = newTagForm }, Cmd.none )
...
switchTag model =
    let
        ...
        oldActive =
            model.tagForm.active

        newTagForm =
            { oldTagForm | active = not oldActive, selectedIds = [] }
    in
